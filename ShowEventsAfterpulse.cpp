...
int main(int argc, char * argv[]){
  ...
  for(int  i_event = 0;i_event<kNumEvent;i_event++){
    ...
    for(int i_cell = 0;i_cell<kNumCell;i_cell++){
      ...
      if (300 <= time[i_cell] and time[i_cell] <= 600
	  and wform1[i_cell] - wform0[i_cell] > 5){
	isAfterPulse = true;
      }
    }
    graph->GetXaxis()->SetLimits(200,800);
